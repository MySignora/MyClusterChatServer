...
vector<Group> GroupModel::queryGroups(int userid){
    ...
    for(int i=0;i<groupVec.size();i++){
        sprintf(sql, "select a.id,a.name,a.state,b.grouprole from user a inner join groupuser b on b.userid = a.id where b.groupid = %d", groupVec[i].getId());
        MYSQL_RES* res = mysql.query(sql);
        if(res != NULL){
            MYSQL_ROW row;
            while((row = mysql_fetch_row(res)) != NULL){
                ...
                user.setState(row[2]);
                user.setRole(row[3]);
                groupVec[i].getUsers().push_back(user);
            }
            mysql_free_result(res);
        }
    }
